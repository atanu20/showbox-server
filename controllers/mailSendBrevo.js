...
const mailer = nodemailer.createTransport({
  ...
  port: 587,
  auth: {
      user: process.env.SEND_BREVO_EMAIL_LOGIN,
       pass: process.env.SEND_BREVO_PASSKEY_LOGIN
  },
});



const mailSendBrevo = (name, to, url, txt, work) => {
  if (work === 'actvation') {
    var email = {
      from: process.env.SEND_BREVO_EMAIL_ADDRESS,
      to: to,
